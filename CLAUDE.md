...
- Phase branches stack: Phase 1 lives on `claude/continuum-crowds-ecs-p85sn0`;
  each subsequent phase branches off the previous phase's branch and its PR
  targets that branch (clean per-phase diffs). Phase 2:
  `claude/cc-phase2-gather-predictive`; Phase 3:
  `claude/cc-phase3-domains-scheduler`; Phase 4:
  `claude/cc-phase4-fim-hybrid`. All changes in this repo only.
...
| v_predictiveDistanceCapCells (spec-added §6.3) | 8 | | | |
...
## Repo layout (as built through Phase 1)

- `Assets/ECSContinuumCrowds/Runtime/` — asmdef `ECSContinuumCrowds`
  (refs Entities, Entities.Hybrid, Transforms, Burst, Collections, Mathematics):
  - `Core/` — `CCConstants` (+Defaults, IsValid, DeriveRhoBar), `GridIndexer`,
    `CCMath` (partial: ENSW tables, splat kernel, world↔grid, HeightGradient) +
    `CCMathSolve.cs` (partial: speed/cost/EikonalSolve/gradients/velocity/
    bilinear), `CCFieldOps` (per-cell f/C, shared job+tests), `CCFmmSolver`
    (static FMM core, shared job+tests) + `CCCellState` flags,
    `IndexedMinHeap` (decrease-key binary heap on native arrays),
    `CCStampOps` (P2: UnitStampData, bucket sizing w/ predictive cap,
    gather cell, gather-side PredictiveWeight closed form + §7.1 rationale,
    active-bucket/cell derivation, scatter reference for tests),
    `CCDomainOps` (P3: BFS flood fill + neighbor table, trigger matrix,
    padded bounds), `CCFimSolver` (P4: per-cell FIM — in-place min-write φ
    per spec §10.2 CPU note, unique-fixed-point determinism argument in the
    class doc; Init/RelaxCell/Compact shared by the parallel sweep jobs and
    the serial finisher; FimRootMode WeightedBlend ships with
    MaxRootWithBlendedPostPass as the §10.4 fallback — hit-cap telemetry
    signals when to flip; max root == EikonalSolve with weights (1,0)).
    P3 note: CCFieldOps/CCFmmSolver/gradients/bilinear all
    operate on COMPACT DOMAINS (local idx + NeighborLocalIdx table + snapshot
    lookups); parity tests drive them through `ParityHarness.IdentityDomain`.
    P4 semantic note: FIM has no accepted set, so it converges to the
    full-information fixed point while repo-FMM's accepted-read-as-∞ quirk
    can leave FMM's φ marginally HIGHER under cost heterogeneity — the
    standing FimParityTests band (1e-2 rel) bounds this; empirical agreement
    is the acceptance bar (spec §10.4).
  - `Components/` — `GlobalFields` + `CCWalkabilityVersion`, `CCWorldConfig` +
    `CCWorldBakeData` blob, `CCUnit` (UnitTag/CCUnit/UnitVelocity/UnitArrived),
    `CCGroup` (CCGroup/GoalCell/CCGroupGoalRect/CCGroupInitialized/
    GroupFieldBuffers + cleanup), `CCSolveSettings` (+GradientScheme,
    CCSolveTick).
  - `Systems/` — `CCSimulationSystemGroup`, `GlobalFieldsInitSystem`,
    `CCConstantsHotReloadSystem` (editor-only), then in update order:
    `CCGroupInitSystem` → `CCUnitSpawnSystem` → `CCSchedulerSystem`
    (P3: replaces CCSolveTickSystem — polls ChainTails/IsCompleted, flips
    ActiveBuffer, stagger slots, trigger eval + telemetry) →
    `CCSpatialHashSystem` (stamping hash + active cells) →
    `CCStampingSystem` (gather; chain stored in `CCStampState`, NOT
    state.Dependency) → `CCDomainSystem` (P3: BFS fill jobs) →
    `CCFieldSystem` → `CCEikonalSystem` (P4: HYBRID — FMM IJob for small
    domains; FIM for domains ≥ FimThresholdCells (placeholder 32768) with
    ≥ FimMinIdleWorkers free: init job → N pre-scheduled parallel
    relax/compact sweep pairs over deferred active lists (no-ops once
    converged) → serial finisher + optional weighted post-pass;
    telemetry FmmSolves/FimSolves/FimHitCaps) →
    `CCVelocityDerivationSystem` (+ back-lookup snapshot publish) →
    `CCAdvectionSystem` (every frame; front snapshot only; escape/stall
    triggers) → `CCMinDistanceSystem` (every frame, own persistent hash).
    P3 chaining rule: solve-chain jobs touch ONLY containers and carry
    explicit deps via `CCGroupSolveState.ChainTail` (⚠ NOTE at its decl:
    deviation from spec §12.4's managed side-table, same intent) so chains
    span frames without entangling per-frame passes; entity-data jobs (hash
    build, advection, min-distance) stay on state.Dependency. WAR guards:
    stamp waits on all ChainTails; hash clear waits on CCStampState.
  - `Authoring/` — `CCConstantsAsset`/`CCConstantsAuthoring`,
    `CCWorldAuthoring`, `CCGroupAuthoring` (goal rect), `CCUnitAuthoring`,
    `CCUnitSpawnerAuthoring`, `CCSolveSettingsAuthoring`.
  - `Debug/` — `CCFieldVisualizer` (ρ/g/walkable/∇h/v̄ + per-group φ/velocity/
    **GradientDiff** shockline diagnostic).
- `Assets/ECSContinuumCrowds/Tests/EditMode/` — asmdef
  `ECSContinuumCrowds.Tests.EditMode`:
  - `ReferenceOracle/` — verbatim repo copies @127cef3 (EikonalSolver,
    DynamicGlobalFields, Constants, Location/FastLocation/Extensions,
    interfaces, TileExtensions) + `FastPriorityQueue` port (BlueRaja
    reference-equality Contains semantics preserved — see below) +
    `OracleTile` single-tile IContinuumTile.
  - `ParityHarness` + `FieldParityTests`/`EikonalParityTests` (φ, velocity,
    corridor divergence) on 8×8 scenarios; `IndexedMinHeapTests`,
    `EikonalSolveTests`, `SpeedCostFieldTests`, `BilinearSampleTests`,
    `WorldIntegrationTests` (P3: walk-around-obstacle through the full
    scheduler machinery + two-group stagger/flip/telemetry test),
    `GatherStampingTests` (P2: gather==scatter parity, bucket sizing,
    active-cell coverage), `PredictiveStampingTests` (P2: threshold gate,
    forward-only, v̄ invariant, scale fade, cap, ghost-chain band,
    continuity), `DomainFloodFillTests`/`DomainTriggerTests` (P3: fill
    correctness incl. canyon-one-domain + trigger matrix),
    `DomainSolveEquivalenceTests` (P3: compact domain ≡ full grid),
    `FimParityTests` (P4 standing FMM↔FIM agreement, both root modes +
    corridor/goal invariants), `CrossoverBenchmarkHarness` (P4 deliverable,
    [Explicit]: FMM-vs-FIM serial wall-time table over graded sizes ×
    uniform/corridor/congested fields — fit the crossover, bake
    FimThresholdCells; caveats in its doc: serial FIM + un-Bursted editor
    calls, use the RATIO), plus the Phase-0 suites.
- `.meta` files are generated by Unity on first project open — commit them then.
...
- **Discovered during Phase 1 (logged per spec §0):** two latent repo behaviors
  worth knowing when reading parity results:
  1. *NaN-skip on fully-walled axes*: when a cell's BOTH neighbors on one axis
     are impassable (C = ∞ both, e.g. a 1-wide corridor), the repo's float
     arithmetic runs `∞ > ∞ → false` into the quadratic, produces NaN, and the
     `NaN < phi` guard silently skips the update — corridor cells never get φ.
     The spec (§9.3) mandates the degenerate-axis 1-D solution instead; we
     implement the spec. Pinned by
     `EikonalParityTests.OneWideCorridor_DivergesFromOracle`.
  2. *Decrease-key never fires in the repo*: `FastPriorityQueue.Contains` uses
     reference equality against `_nodes[QueueIndex]`, and the repo constructs a
     NEW FastLocation per neighbor visit — so `Contains` is always false and
     duplicates are enqueued instead (stale pops are no-ops under the monotone
     φ guard). Our `IndexedMinHeap` does exact decrease-key; final φ agrees
     (parity-tested). The oracle port reproduces the repo behavior faithfully.
