...
- `Assets/ECSContinuumCrowds/Runtime/` — asmdef `ECSContinuumCrowds`
  ...
  - `Core/` — `CCConstants` (+Defaults, IsValid, DeriveRhoBar), `GridIndexer`,
    `CCMath` (ENSW tables, splat kernel scatter+gather, world↔grid, HeightGradient).
  - `Components/` — `GlobalFields` + `CCWalkabilityVersion`, `CCWorldConfig` +
    `CCWorldBakeData` blob.
  - `Systems/` — `CCSimulationSystemGroup`, `GlobalFieldsInitSystem` (one-shot
    alloc + DH/walkable bake, owns disposal), `CCConstantsHotReloadSystem`
    (#if UNITY_EDITOR, managed SystemBase, AssetDatabase scan).
  - `Authoring/` — `CCConstantsAsset` (SO), `CCConstantsAuthoring` (baker),
    `CCWorldAuthoring` (baker: textures → blob; origin = transform XZ).
  - `Debug/` — `CCFieldVisualizer` (gizmo window: ρ/g/walkable/∇h/v̄ modes).
- `Assets/ECSContinuumCrowds/Tests/EditMode/` — asmdef
  `ECSContinuumCrowds.Tests.EditMode`; GridIndexer, splat-kernel invariants
  (ρ̄ assert), constants-vs-repo, height-gradient tests.
