...
- Phase branches stack: Phase 1 lives on `claude/continuum-crowds-ecs-p85sn0`;
  ...
  `claude/cc-phase2-gather-predictive`. All changes in this repo only.
...
- `Assets/ECSContinuumCrowds/Runtime/` — asmdef `ECSContinuumCrowds`
  ...
  - `Systems/` — `CCSimulationSystemGroup`, `GlobalFieldsInitSystem`,
    ...
    `CCSolveTickSystem` → `CCGroupInitSystem` → `CCUnitSpawnSystem` →
    `CCSpatialHashSystem` (P2: stamping hash + active cells, owns
    `CCStampingHash` singleton) → `CCStampingSystem` (P2 gather; P1 scatter
    system retired, semantics live in CCStampOps.Scatter* as test oracle) →
    `CCFieldSystem` →
    `CCEikonalSystem` (FMM IJob/group) → `CCVelocityDerivationSystem` →
    `CCAdvectionSystem` (every frame) → `CCMinDistanceSystem` (every frame,
    own persistent hash).
...
- `Assets/ECSContinuumCrowds/Tests/EditMode/` — asmdef
  ...
  - `ParityHarness` + `FieldParityTests`/`EikonalParityTests` (φ, velocity,
    ...
    `WorldIntegrationTests` (full-pipeline walk-around-obstacle),
    ...
    continuity), plus the Phase-0 suites.
