...
- Phase branches stack: Phase 1 lives on `claude/continuum-crowds-ecs-p85sn0`;
  ...
  `claude/cc-phase3-domains-scheduler`. All changes in this repo only.
...
- `Assets/ECSContinuumCrowds/Runtime/` — asmdef `ECSContinuumCrowds`
  ...
  - `Core/` — `CCConstants` (+Defaults, IsValid, DeriveRhoBar), `GridIndexer`,
    ...
    padded bounds). P3 note: CCFieldOps/CCFmmSolver/gradients/bilinear all
  ...
  - `Systems/` — `CCSimulationSystemGroup`, `GlobalFieldsInitSystem`,
    ...
    `CCFieldSystem` → `CCEikonalSystem` (FMM IJob/group) →
